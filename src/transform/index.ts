...
const YinBiao = 'YinBiao'
const ShengDiao = 'ShengDiao'
const ShiYi = 'ShiYi'

// 手动定义表头
const headers = [YinBiao, ShengDiao, ShiYi]; // 根据你的实际情况修改
...
function readTsv(filePath): Promise<CharList> {
...
function saveTsv(data: CharList, fileName: string) {
...
function convertAndSaveTianPing(data: CharList) {
  data.forEach(ele => {
    if (
  ...
  saveTsv(data, `浏阳田坪.tsv`)
}

async function main() {
  const data = await readTsv(filePath)
  convertAndSaveTianPing(data)
